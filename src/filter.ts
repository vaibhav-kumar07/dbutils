...
export const applyPaginationFilter = (filters: any, pagination: any, sort: string) => {
	...
	if (filters) {
		for (const [field, condition] of Object.entries(filters) as [string, Record<string, any>][]) {
			...
			if (operator && value !== undefined) {
				...
				} else if (operator.toLowerCase() === '$between' && typeof value === 'string') {
...
export const endOfDay = (strDate: string, format: string = 'YYYY-MM-DD'): Date => {
	return moment(strDate, format).endOf('day').toDate();
};

export const startOfDay = (strDate: string, format: string = 'YYYY-MM-DD'): Date => {
